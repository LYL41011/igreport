			...
			public void run() {
				...
				while (!toStop) {
					try {
						...
						List<Long> failLogIds = XxlJobAdminConfig.getAdminConfig().getIgReportJobLogDao().findFailJobLogIds(1000);
						if (failLogIds!=null && !failLogIds.isEmpty()) {
							for (long failLogId: failLogIds) {

								// lock log
								int lockRet = XxlJobAdminConfig.getAdminConfig().getIgReportJobLogDao().updateAlarmStatus(failLogId, 0, -1);
								if (lockRet < 1) {
									continue;
								}
								XxlJobLog log = XxlJobAdminConfig.getAdminConfig().getIgReportJobLogDao().load(failLogId);
								XxlJobInfo info = XxlJobAdminConfig.getAdminConfig().getIgReportJobInfoDao().loadById(log.getJobId());
								...
								if (log.getExecutorFailRetryCount() > 0) {
									...
									XxlJobAdminConfig.getAdminConfig().getIgReportJobLogDao().updateTriggerInfo(log);
								}
								...
								XxlJobAdminConfig.getAdminConfig().getIgReportJobLogDao().updateAlarmStatus(failLogId, -1, newAlarmStatus);
							}
						}
